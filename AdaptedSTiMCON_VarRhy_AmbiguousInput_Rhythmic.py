...
Implement extended STiMCON when presented with isochronous input, 
...
delays = np.linspace(-0.25,0.75,20)/Freq*fs # delays has to be changed with stim freq; but kept here bc length is needed
#ks = [0,10,20,30,40]
...
for iteration in range(0,iterations):
    ...
    for iF in range(len(stim_Freqs)):
        ...
        for cntProp in range(len(prop)): #12
            ...
            for cntDel in range(len(delays)): #20
                 ...
                 #10062024: staying with the most predictable two nodes
    ...
    np.save(file=r"data_VarRhy_AllFirstSpTime_RhythmicInput_it{}.npy".format(iteration+1),arr=AFST_it)
    np.save(file=r"data_VarRhy_AllFirstSpTime_rel_RhythmicInput_it{}.npy".format(iteration+1),arr=AFST_relStimOnset_it)
    np.save(r"data_VarRhy_FirstActive_RhythmicInput_it{}.npy".format(iteration+1),arr=FA_it)
    SI_it = SensoryInput_Timing[:,:,iteration,:,:]
    np.save(r"data_VarRhy_SensoryInput_RhythmicInput_it{}.npy".format(iteration+1),arr=SI_it)
    np.save("data_VarRhy_Phases_RhythmicInput_it{}.npy".format(iteration+1),arr=Phase)